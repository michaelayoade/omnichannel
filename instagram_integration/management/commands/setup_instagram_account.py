    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            try:
                ...
                if options["subscribe_webhook"]:
                    if not webhook_url:
                        ...
                        try:
                            ...
                            self.stdout.write(
                                self.style.ERROR(
                                    f"Failed to subscribe to webhooks: {str(e)}"
                                )
                            )
                ...
                self.stdout.write(
                    self.style.ERROR(f"Failed to fetch account info: {str(e)}")
                )

        except Exception as e:
            raise CommandError(f"Failed to set up Instagram account: {str(e)}")
